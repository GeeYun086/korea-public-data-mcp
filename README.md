...
대한민국 공공데이터를 Claude가 **직접 조회해서 답하게** 해주는 MCP 입니다.
...
| **교육** | ✅ 완료 | NEIS |
| 학술 / 연구 | 🔶 일부 | 서울연구원(완료) · NTIS(신청자 개인 IP에서만 동작) · KCI(코드 완료, 키 발급 보류) · KISTI(승인 대기) |
| 인력 / 채용 | 🔶 일부 | 국민연금(완료) · 원티드(코드 완료, 키 발급 보류) · 고용24 KDT(코드 완료, 키 발급 보류) |

상태 표기: ✅ 코드 완료 + 키 발급 + 정상 호출 확인 · ⏸ 코드 완료, 키 발급 대기/보류 ·
⚠️ 동작은 하지만 조건부(특정 IP·별도 신청 등) · ⛔ 제외.
...
| 기관 | 제공 데이터 | 상태 | 출처 |
| --- | --- | --- | --- |
| 국민연금공단 | 회사명으로 사업장 검색 → 가입자수(재직자수 근사치) 및 신규취득·상실 추이 | ✅ 완료 (실호출로 확인) | [data.go.kr 3046071](https://www.data.go.kr/data/3046071/openapi.do) |
...
✅ 발급 완료·정상 동작 / ⏸ 코드는 완료, 키 발급 대기·보류 / ⚠️ 동작은 하지만 조건부(특정 IP·별도 신청 등).
...
| `ntis_search_projects` | NTIS로 국가 R&D 과제를 키워드로 검색합니다. ⚠️ 지금 키는 신청자 개인 공인 IP로만 등록돼 있어 그 사람 환경에서만 동작합니다 — 팀 공용은 고정 IP 서버 배포 + 등록 IP 갱신 필요 |
...
| 도구 | 설명 |
| --- | --- |
| `nps_search_employee_count` | 회사명으로 국민연금 사업장을 찾아 가입자수(재직자수 근사치)를 조회합니다 |
