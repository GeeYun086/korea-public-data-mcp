...
def register(mcp: MCPServer) -> None:
    ...
    async def neis_search_schools(
        ...
        limit: 최대 반환 건수 (최대 1000). 인증키가 없으면 5건으로 강제 제한된다.
        """
        key = f"neis:{region}:{school_name}:{school_kind}:{limit}"
        try:
            return await cached_call(
                key, lambda: neis_client.search_schools(region, school_name, school_kind, limit)
            )
    ...
    async def ip_search(query: str, limit: int = 20, page: int = 1) -> dict:
        ...
        """
        key = f"kipris:{query}:{limit}:{page}"
        try:
            return await cached_call(key, lambda: kipris_client.search_patents(query, limit, page))
        except MissingApiKeyError as e:
            return {"error": str(e)}
